...
interface RecipeListPreviewProps {
  recipes: RecipeData[];
  favouritesMap: Record<number, number>;
}

export const RecipeListPreview = ({
  recipes,
  favouritesMap,
  ...
  useEffect(() => {
    const searchRecipes = async (term: string) => {
      const response = await fetch(
        `http://127.0.0.1:8000/api/recipes?title=${term}`
      );
          ...
          >
            <div>
              <img
                src={
                  recipe.cover_image
                    ? `${process.env.NEXT_PUBLIC_BACKEND_URL}/${recipe.cover_image.pathname}`
                    : `${process.env.NEXT_PUBLIC_BACKEND_URL}/uploads/default-recipe-image.jpeg`
                }
                alt={recipe.title}
